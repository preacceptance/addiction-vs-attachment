#!/usr/bin/env python3
"""
Deeper (LLM) coding pass over the legal corpus.

One model call per paragraph: is the phenomenon described addiction,
attachment, both, or neither. The coding manual is the system prompt and the
curated few-shot exemplars precede each paragraph. Each pass runs with a fixed
API seed set by RUN_TAG (pass 1 = seed 1, pass 2 = seed 2); pass 2 is the
production run. Responses cache to a JSONL file, so an interrupted run resumes.

Input:  legal_paragraphs_24.xlsx, ../3_fewshots/fewshots_RESOLVED_grid.xlsx
Output: legal_paragraphs_24_llm<RUN_TAG>.xlsx
"""
...
CORPUS     = ROOT / "output" / "legal_paragraphs_24.xlsx"
FEW_SHOTS  = ROOT / "modified_data" / "fewshots_RESOLVED_grid.xlsx"
FEWSHOT_SHEET = "Few-shot set"
OUTPUT     = ROOT / "output" / f"legal_paragraphs_24_llm{RUN_TAG}.xlsx"
CACHE      = ROOT / "output" / f".cache_legal_llm{RUN_TAG}.jsonl"
...
def _norm(s) -> str:
    """Whitespace-normalised text, for comparison only.

    Exemplar texts may differ from the corpus in line breaks and runs of
    whitespace (spreadsheet transport); any other difference means a genuinely
    different paragraph and is treated as an error.
    """
...
def load_fewshots(corpus: pd.DataFrame) -> pd.DataFrame:
    ...
    claims to be: every unit_id must exist in the corpus, appear only once,
    and carry text matching the corpus (up to whitespace).
    """
    ...
    text_by_id = dict(zip(corpus["unit_id"], corpus["text"]))
    ...
    if drifted:
        raise SystemExit(f"{len(drifted)} few-shot paragraphs no longer match the "
                         f"corpus text: {drifted[:5]}")
...
def code_paragraph(client: OpenAI, text: str,
    ...
    messages = [{"role": "system", "content": LEGAL_CODING_SYSTEM}]
    messages += fewshot
...
def main() -> None:
    if LEGAL_CODING_SYSTEM.startswith("PLACEHOLDER"):
    ...
    if not CORPUS.exists():
        raise SystemExit(f"{CORPUS.name} not found — run legal_surface_24.py first.")

    df = pd.read_excel(CORPUS)
    assert df["unit_id"].is_unique, "unit_id is not unique in the corpus"
    fs = load_fewshots(df)
    ...
    assert int(df["is_fewshot"].sum()) == len(fs), \
        "few-shot rows did not all match the corpus on unit_id"

    df["deeper_meaning"] = ""
    df["reasoning"] = ""

    cached = read_cache()
    todo = [i for i in df.index[~df["is_fewshot"]]
            if df.at[i, "unit_id"] not in cached]

    print(f"\nLEGAL{RUN_TAG}: {len(df):,} paragraphs · "
          f"{int(df['is_fewshot'].sum())} few-shot rows excluded · "
          f"{len(cached):,} already cached · {len(todo):,} to code")
    ...
    if todo:
        ...
        def task(idx: int):
            ...
            res = code_paragraph(client, str(df.at[idx, "text"]), fewshot)
            rec = dict(unit_id=uid,
        ...
        with ThreadPoolExecutor(max_workers=CONCURRENT_WORKERS) as ex:
            futs = [ex.submit(task, i) for i in todo]
            for fut in tqdm(as_completed(futs), total=len(futs), desc=f"LEGAL{RUN_TAG}"):
                rec = fut.result()
                cached[rec["unit_id"]] = rec

    for i in df.index[~df["is_fewshot"]]:
        rec = cached.get(df.at[i, "unit_id"])
        if rec:
...
def _summarise(df: pd.DataFrame) -> None:
    ...
    print("\nby case (substantive only)")
    sub = coded[coded["deeper_meaning"] != "neither"]
    print(pd.crosstab(sub["case"], sub["deeper_meaning"]).to_string())
